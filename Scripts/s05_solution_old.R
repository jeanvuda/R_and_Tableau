...
cc[role.x==x & !is.na(class),.(DS, role.y, folder.name, caption, datatype, var, nformula, Checked, Definition, Duplicate, Suggestion)]})
...
cd<-lapply(c("dimension", "measure"), function(x){
  cd[role.y==x & is.na(class),.(DS, role.y, folder.name, caption, datatype, var)]})
